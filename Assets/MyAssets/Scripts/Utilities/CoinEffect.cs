...
public class CoinEffect : MonoBehaviour
{
    ...
    [SerializeField] private float deltaDown = 1f;
    ...
    public void TestCoin(UnityAction action = null)
    {
        this.AddCoins(0, 10,
            () => {
                if (action != null)
            ...
            if (this.coinQueue.Count > 0)
            {
                ...
                coin.transform.position = collectedCoinPosition +
                                          new Vector3(UnityEngine.Random.Range(-this.spread / 2, this.spread / 2), 0f,
                                              0f);
                // Hiệu ứng shake
                coin.transform.DOMoveY(coin.transform.position.y - deltaDown, timeDown)
